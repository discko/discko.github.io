...
先说结论。对volatile变量的写操作，在底层是通过StoreLoad屏障，而StoreLoad是在汇编指令上增加lock前缀从而达成可见性与有序性的功能的。对于volatile的读操作和非volatile的读写操作不加lock。  
更详细的说明可以阅读我单独写的[volatile的底层实现](/2021/02/02/Concurrent-VolatileImplementation/)。
